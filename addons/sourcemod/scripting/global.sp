...
{
	name = "LumiAdmin Global", 
	author = "DanZay, LumiAdmin", 
...
#include "global/legacy_disable.sp"
#include "global/api.sp"
#include "global/ban_player.sp"
#include "global/commands.sp"
#include "global/maptop_menu.sp"
#include "global/print_records.sp"
#include "global/send_run.sp"
#include "global/points.sp"
...
public void OnPluginStart()
{
	...
	{
		SetFailState("global currently only supports 128 tickrate servers.");
	}
	if (FindCommandLineParam("-insecure") || FindCommandLineParam("-tools"))
	{
		SetFailState("global currently only supports VAC-secured servers.");
	}
	...
	{
		SetFailState("Failed to disable legacy gokz-global.smx. Move it to plugins/disabled and reload global.");
	}

	if (gB_LegacyGlobalSurfaceDeferred)
	{
		ServerCommand("sm plugins reload global");
		return;
	}
...
static void ApplySettingsEnforcerForLateLoad()
{
	...
	for (int client = 1; client <= MaxClients; client++)
	{
		...
		{
			LogMessage("Invalidated %N's active run because global was loaded after map start.", client);
			GOKZ_InvalidateRun(client);
		}
	}
